...
export class ParameterService {
  ...
  getPersonCardPositions(forceRefresh?: boolean) {
    return this.utilsService.cache_results(
      this.personCardPositions$,
      `/person_card_positions`,
      forceRefresh
    );
  }
  ...
  saveBranchProduct(branch_id, branch_product) {
    return this.http.post(
      this.dataUrl + `/branch_products/${branch_id}`,
      branch_product
    );
  }
  ...
  archiveBranchProduct(product) {
    return this.http.post(
      this.dataUrl + `/branch_products/archive/${product.branch_id || 0}`,
      product
    );
  }
  ...
  saveVoucher(voucher) {
    return this.http.post(this.dataUrl + `/parameters/vouchers`, {
      voucher
    });
  }

  ToggleAssociateBranchAcquirer(branch_id, acquirer_id) {
    return this.http.post(this.dataUrl + `/branches_acquirers`, {
      branch_id,
      acquirer_id
    });
  }
  ...
  getAvailableShirtSizes() {
    return [
      ...
      { size: "GG", name: "GG" },
      { size: "GGG", name: "GGG" },
      { size: "IP", name: "Infantil - P" },
      { size: "IM", name: "Infantil - M" },
      { size: "IG", name: "Infantil - G" }
    ];
  }

  getAvailablePantsSizes() {
    return [
      { size: "P", name: "P" },
      { size: "M", name: "M" },
      { size: "G", name: "G" },
