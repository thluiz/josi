...
export class ProjectPageComponent implements OnInit, OnDestroy {
    ...
    dragulaService.drop.subscribe((value) => {
      ...
      if(destination > 0 && target > 0) {
        if(origin != destination) {
          this.cardService.saveCardStep(target, destination).subscribe(() => console.log('OK!'));
        }
  ...
  private load_project() {
    this.cardService.getProject(this.id)
    .subscribe((result_project: any) => {
      this.project = result_project.data;
      this.getProjectCommentaries(this.project);
    });
  }

  private getProjectCommentaries(project: any) {
    this.cardService.getCardCommentaries(project)
    .subscribe((result_commentaries: Result<CardCommentary[]>) => {
