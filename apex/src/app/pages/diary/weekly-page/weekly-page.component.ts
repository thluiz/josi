...
export class WeeklyPageComponent implements OnInit, OnDestroy {
  ...
  private load_external_people(result) {
    ...
    if(this.current_branch == 0) {
      external_people = result.people.filter(p => p.is_interested
        || p.is_service_provider || p.is_inactive_member
      ...
      external_people = result.people.filter(p => p.is_interested || p.is_service_provider
        || p.is_inactive_member || p.is_associated_with_member
        || p.is_external_member || p.branch_id != this.current_branch);
    }
