...
export class MoveCardModalComponent implements OnInit {      
  ...
  open(parameter : Card) { 
    this.card = parameter;   
    this.cardService.getFlatOrganizationsData().subscribe((data: any[]) => {
      this.organizations = data;

      if(this.card.organization_id > 0) {
        // card in subproject
        this.new_organization = data.find(d => d.id == this.card.organization_id);
  ...
  move_card(close_action) {
    ...
    this.cardService.moveCard(this.card, parent_id, step_id).subscribe(data => {
      if(close_action) {
        close_action();
      }
    });    
