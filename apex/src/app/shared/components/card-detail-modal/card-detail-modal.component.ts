...
export class CardDetailModalComponent implements OnInit {  
  ...
  ngOnInit() {        
    ...
    .filter((ca) => ca.type == CARD_COMMENT_ADDED)
    .subscribe((action) => {
      if(!this.card)
        return; 
      
      let commentaries = action.payload as CardCommentary[];

      this.commentaries = commentaries.filter(cc => cc.card_id == this.card.id).sort(cc => cc.id);
    });    
  }  
