...
export class PersonFinancialTreatmentModalComponent implements OnInit {
  ...
  open(person) {
    ...
    observableZip(
      ...
      (result_person_data: Result<any>,
        ...
        this.without_schedule_payments = result_pending_data.data.pending[0].without_schedule_payments;
  ...
  private load_data() {
    this.personService.getPendingFinancial(this.person_id())
    .subscribe((missing_data : Result<any>) => {
      this.pending = missing_data.data as any;
    });
  }
