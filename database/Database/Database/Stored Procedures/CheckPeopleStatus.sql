...
begin                      
 ...
 and p.id not in (select distinct pic.id from person_incident pic                       
       join incident i on pic.incident_id = i.id                      
       where  i.incident_type = 24                      
        and i.closed = 0                      
        and i.treated = 0                   
 ...
 insert into @people_scheduling_status(person_id, [status])                      
  select p.Id, 2                      
  from person p                        where p.id = isnull(@person, p.id)  
 and (p.is_active_member = 1 or p.is_leaving = 1 or p.is_inactive_member = 1)                              
 ...
 exec GenerateBirthDateIncidents @person 
