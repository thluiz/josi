...
as  
begin   
 declare @start_date date = datefromparts(datepart(year, getdate()), datepart(month, getdate()), 1)  
 ...
 join   
  ...
  where i.incident_type = 10  
   and i.closed = 1  
   and i.treated = 0  
   and cast(date as date) between datefromparts(datepart(year, getdate()), datepart(month, getdate()), isnull(datepart(day, enrollment_date), 1)) 
								and dateadd(month, 1, datefromparts(datepart(year, getdate()), datepart(month, getdate()), isnull(datepart(day, enrollment_date), 1)))
   and p.program_id = 1    
